...
int main(int argc, char** argv) {
    ...
    // const int clientPort = 8011;// 指定客户端端口
    ...
    if (connect(fd, ( struct sockaddr* )&server_addr, sizeof(sockaddr_in))
        == -1) {
    ...
    while (true) {
        ...
        if (totalSize > 6291456) /* 62914560=60*1024*1024=60mb*/
        {
            ...
            if (t2 - t1 > 0) {
                ...
                printf(
                    "fd=%d,size=%d,totalSize=%llu,speed=%lluMbps,content=%s\n",
                    fd, size, totalSize, speed, buf);
                totalSize = 0;
                t1        = time(NULL);
            }
        }
